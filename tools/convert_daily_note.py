...
def sanitize_public_text(text: str) -> str:
    ...
    for raw in (text or "").splitlines():
        ...
        line = line.replace("加仓暴露", "提高仓位暴露")
        line = re.sub(r"(?<!增)加仓", "提高暴露", line)
        ...
        line = re.sub(
            r"(中科曙光|中科|歌尔股份|歌尔|金山办公|金山|海光信息|海光|中信证券|中信|药明康德|药明|海兰信|南大光电|南大|雅克科技|雅克|澜起科技|中微公司|聚和材料|聚和|太极实业|太极|徐工机械|徐工|华工科技|华工|江丰电子|江丰|金宏气体|金宏|长电科技|长电|兆易创新|兆易|方正科技|方正|通富微电|新易盛|中际旭创|天孚通信|中国卫星|上海瀚讯|盛路通信|通宇通讯|通宇|深信服|中安科|中大力德|东信和平|东信|紫光股份|紫光|京东方A)",
            "标的",
            line,
        )
        line = re.sub(r"账户因标的\s*[与和、/]\s*标的回撤", "账户因持仓回撤", line)
        line = re.sub(
            r"标的分批兑现并降低风险，标的只验证",
            "部分持仓完成风险兑现，新增持仓只验证",
            line,
        )
        line = re.sub(r"(?:标的\s*[/、和]\s*)+标的([两三四五]只)持仓", r"\1持仓标的", line)
        line = re.sub(r"(?:标的\s*/\s*)+标的", "持仓标的", line)
        line = re.sub(r"(?:标的\s*[、和]\s*)+标的", "持仓标的", line)
        line = re.sub(r"标的和标的", "持仓标的", line)
        line = re.sub(r"持仓持仓标的", "持仓标的", line)
